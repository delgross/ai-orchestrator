...
class ToolExecutor:
        ...
        return {
            ...
            "exploration": [
                ...
                },
                {
                    ...
                    "function": {
                        "name": "consult_advice",
                        "description": "Consult the Advice Subsystem/Critic. Use this to get second opinions or critical analysis.",
                        "parameters": {
                            "type": "object", 
                            "properties": {
                                "topic": {"type": "string", "description": "Topic to get advice on"},
                                "query": {"type": "string", "description": "Alternative to topic"}
                            }, 
                            "required": []
                        }
                    }
    ...
    def get_core_menu(self) -> str:
        """
        [LATENCY OPTIMIZATION]
        Generates a concise 'Core Menu' of tools for the Maître d'.
        Excludes explicit 'admin', 'cleanup', and 'auto_discovered' (orphans) 
        categories to reduce prompt size (~800 tokens vs 3000 tokens).
        
        The excluded tools remain accessible via Vector Search.
        """
        ALLOWED_CATEGORIES = {
            "thinking",   # Critical
            "core",       # Time, Location
    ...
    async def get_all_tools(self, messages: Optional[List[Dict[str, Any]]] = None, precomputed_intent: Optional[Dict[str, Any]] = None, quality_tier: Optional['QualityTier'] = None, precomputed_router_analysis: Optional[Any] = None, capability_analysis: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        ...
        # We must deduplicate strictly by name before any further processing.
        unique_tools = {}
        for t in tools:
            ...
            # Priority: Keep the first one encountered (usually Core/Native)
            if t_name not in unique_tools:
                unique_tools[t_name] = t
        
        final_tools = list(unique_tools.values())
        if len(final_tools) != len(tools):
            logger.info(f"✅ Final Tool Count: {len(final_tools)} (Deduplicated)")
            
        # [FIX] Sort tools deterministically for Prefix Caching stability
        final_tools.sort(key=lambda x: x["function"]["name"])

        self.tool_definitions = final_tools
        tools = final_tools # [FIX] Ensure downstream logic uses deduplicated list     
        # The following logic (capability enhancement, router analyzer, quality tier filtering)
        # will now operate on 'final_tools' instead of 'tools'.
