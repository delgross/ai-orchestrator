...
async def main():
    ...
    async def list_tools() -> list[Tool]:
        return [
            Tool(name="get_system_health", description="Check router/agent/MCP health and uptime; returns full topology.", inputSchema={"type":"object","properties":{}}),
            Tool(name="list_active_mcp_servers", description="List MCP servers with status, failures, and tool counts.", inputSchema={"type":"object","properties":{}}),
            ...
            Tool(name="read_service_logs", description="Tail router/agent/ollama logs (last N lines).", inputSchema={"type":"object","properties":{"service":{"type":"string","enum":["router","agent_runner","ollama"]}, "lines":{"type":"integer","default":20}},"required":["service"]}),
            Tool(name="check_resource_usage", description="Check CPU/RAM for orchestration processes and disk usage.", inputSchema={"type":"object","properties":{}}),
            ...
            Tool(name="add_mcp_server", description="Add/update an MCP server (cmd/env); reload and discover tools.", inputSchema={"type":"object","properties":{"name":{"type":"string"},"config":{"type":"object"}},"required":["name","config"]}),
            Tool(name="install_mcp_package", description="Install an MCP via npm package name (optionally with args).", inputSchema={"type":"object","properties":{"package":{"type":"string"},"name":{"type":"string"},"args":{"type":"array","items":{"type":"string"}}},"required":["package"]}),
